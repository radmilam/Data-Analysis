...
-------------------------------
Simulated 1000 data sets, x, y have no relationship. Take coefficient matrix, 2nd row, 4th column is the p-value for the relationship y~x.
...
What happens if we adjust the p values and apply FFWE with the false discovery?
```{r cache=TRUE}
# Controls FWER 
sum(p.adjust(pValues,method="bonferroni") < 0.05)
```
After comparing the corrected p-values to standard alpha level, we find there's no true positives and we would find no or few significant results, which is good because there's no relationship.

Same with Benjamini–Hochberg (BH) correction.
```{r cache=TRUE}
# Controls FDR 
...
```
About 5% of the time we get a false positive (24). All the values for cases where there is a relationship are less than 0.05, so we discover all the signals that exist in this data set.

Using FWER we miss 23 of the cases where there should be a singnal, but now, we have no false positives because we're controling the probability of even one false positive to be less than 0.05 in this case.
```{r cache=TRUE}
# Controls FWER 
table(p.adjust(pValues,method="bonferroni") < 0.05,trueStatus)
```

With FDR we discover fewer false positives.
```{r cache=TRUE}
# Controls FDR 
...
In case of BF, we're multiplying the p-values by the number of tests that are performed.
```{r cache=TRUE}
par(mfrow=c(1,2))
...
---------------------
Evaluate whether violation of your assumption leads to differences in sort of your statistical analysis.
```{r cache=TRUE}
set.seed(44333)
x <- rnorm(50)
# linearly distributed - standard assumption for lin reg
e <- rnorm(50)
# have fatter tail - much larger number of large values
e2 <- rcauchy(50)
b0 <- 1
b1 <- 2
y <- b0 + b1*x + e
...
plot(lm(y2 ~ x)$fitted,
     ...
     ylab="residuals")
```
In the diagram with cauchy error terms, we get few large values, which suggsts that our model isn't fitting for a small number of observations, which in turn suggests there may be a problem with the model.

Repeated simulations
--------------------
To get an idea of the variability of the studies which may give an idea of how robust the answers you're getting are in your individual dataset.
```{r cache=TRUE}
set.seed(44333)
...
for(i in 1:1000){
 ...
 # regression coefficients for normal and cauchy errors
 betaNorm[i] <- lm(y ~ x)$coeff[2]; 
 betaCauch[i] <- lm(y2 ~ x)$coeff[2]
}
quantile(betaNorm)
```
Quantiles for normal errors are centered around 2, which is good because we generated data that had a slope of 2 and they have a reasonable variation.

```{r cache=TRUE}
quantile(betaCauch)
```
We get some estimates that can be wildly different. If we believe that our error terms are closer to Cauchy than they are to normal, it may lead to high variability/mistrust of the beta estimates that we get from our regression modeling.

Monte Carlo Error
-----------------
Left: normal, right: Cauchy. Gives an idea how much more variable our estimate would be if we had a change in the sort of model assumptions in terms of the error distribution that we're using.

We can reduce the Monte Carlo error by increasing the number of samples that we collect.
...
------------------------------
want to take some of the characteristics of the data into account when generating data to evaluate modeling assumptions.
s
```{r cache=TRUE}
...
---------------------------
We may want to generate data that come from similar models to the model that you observed in actual data.
```{r cache=TRUE}
...
child0 <- lm1$coeff[1] + 
  lm1$coeff[2]* 
  parent0 + 
  rnorm(nobs, # noise - sd is the amount of variability that
              # we'd observe in residual variation after fitting
              # a linear model.
...
library(bootstrap)
```
Stamp data - thickness of a large number of stamps from a particular year.
```{r cache=TRUE}
data(stamp);
nobs <- dim(stamp)[1]
...
--------------------------------
Plotting density of the original set and newThick values that have the same mean, sd.
```{r cache=TRUE}
...
```
Each one of the grey density lines represents one new data set, they're all missing the key characteristic of the original data set, the two humps.

How density estimation works
----------------------------
Slide 12/15
We go back to original data and look at how the density estimate was calculated so we can simulate it.

The black lines at the bottom are data points. For each data point, we can plot normal distribution centered on that point, where the standard deviation is a fixed value. To get the one density, we add up the 6 (in this case) densities at x values - this is what **kernel density** does. We can simulate data, where the data is drawn randomly from one of these small normal distributions.

Simulating from the density estimate
-------------------------------------
Black plot is the of the original data. `nobs` is a new set of observations, with mean equal to stamp thickness and standard deviation to `bw`, bandwidth that we get from the original density estimate in original data. The bandwidth is defined as the standard deviation of the small little normal distributions that you were going to add up.
```{r cache=TRUE}
plot(density(stamp$Thickness),col="black",lwd=3)
for(i in 1:10){
...
-----------------------
If we think the curve is matched too closely to data.
We can generate variables that have the same means but we increased sd by some factor, say 50%.
```{r cache=TRUE}
plot(density(stamp$Thickness),col="black",lwd=3)
for(i in 1:10){
